...
### 2026-06-22 — Hosted deploy live; local launcher simplified; bartender/customer fixes
- The app is **hosted** (Cloudflare Pages frontend + Render backend); other devices / the demo use the **hosted URL**. Locally, ngrok is still used to give MP a public return URL (see payments below).
- `start-fastbuy.ps1` runs the backend (:8080, `local` profile) + Vite (:5173) + an **ngrok tunnel** (static domain `crepe-phonics-slogan.ngrok-free.dev`) and opens the ngrok URL. ngrok is required locally because MP `auto_return` needs a public https URL to redirect back to — it cannot return to `localhost`. (An earlier same-day attempt to drop ngrok and run local-only was reverted for this reason.)
...
- **Customer order persistence:** customer-flow state moved from sessionStorage → localStorage (`loadPersisted/savePersisted/clearPersisted`, keys `PERSIST_*`) so reopening on the same device restores the in-progress order. Transient checkout screens (`payment`/`paying`/`rejected`) are never auto-resumed (a restored `paying` re-fired payment with an empty cart → instant fail).
...
- (Merged into **production** on 2026-06-22; the `production` branch keeps its deploy config + dev-button removals on top of these.)
